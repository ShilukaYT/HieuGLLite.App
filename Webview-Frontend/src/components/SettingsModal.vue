  ...
  <v-dialog v-model="isOpen" max-width="800" height="600" transition="dialog-bottom-transition" scrollable persistent
    ...
    <v-card class="rounded-xl overflow-hidden" :color="isDark ? '#121212' : '#FFFFFF'" :style="{
      ...
      <v-toolbar color="transparent" flat class="px-4 mt-2">
        <v-toolbar-title class="text-h5 font-weight-bold"><v-icon icon="mdi-cog-outline"></v-icon> Cài đặt hệ thống</v-toolbar-title>
        <v-spacer></v-spacer>
      ...
      <div class="d-flex flex-row h-100" style="min-height: 400px;">
        ...
        <v-tabs v-model="tab" direction="vertical" color="warning" class="px-2" style="width: 180px;">
          <v-tab value="general" class="justify-start rounded-lg mb-1"><v-icon start icon="mdi-cog-outline"></v-icon> Chung</v-tab>
          <v-tab value="update" class="justify-start rounded-lg"><v-icon start icon="mdi-update"></v-icon>Cập nhật</v-tab>
          <v-tab value="application" class="justify-start round-lg"><v-icon start icon="mdi-application"></v-icon>Ứng dụng</v-tab>
          <v-tab value="about" class="justify-start rounded-lg"><v-icon start icon="mdi-information-outline"></v-icon> Giới thiệu</v-tab>
        </v-tabs>
        ...
        <v-window v-model="tab" class="flex-grow-1 pa-6">
          <v-window-item value="general">
            ...
            <v-list bg-transparent class="pa-0">
              <v-list-item class="px-0">
                ...
                <template v-slot:append>
                  <v-btn-toggle v-model="themePreference" mandatory color="warning" variant="tonal" rounded="pill" @update:model-value="applyThemePreference">
                    <v-btn value="light" size="small"><v-icon icon="mdi-white-balance-sunny" class="mr-1"></v-icon> Sáng</v-btn>
                    <v-btn value="dark" size="small"><v-icon icon="mdi-moon-waning-crescent" class="mr-1"></v-icon> Tối</v-btn>
                    <v-btn value="system" size="small"><v-icon icon="mdi-monitor" class="mr-1"></v-icon> Hệ thống</v-btn>
                  </v-btn-toggle>
                </template>
              </v-list-item>
            </v-list>
            ...
            <v-list bg-transparent class="pa-0">
              <v-list-item class="px-0">
                ...
                <template v-slot:append>
                  <v-btn-toggle v-model="closeBehavior" mandatory color="primary" variant="tonal" rounded="pill" @update:model-value="applyCloseBehavior">
                    <v-btn value="tray" size="small"><v-icon icon="mdi-tray-arrow-down" class="mr-1"></v-icon> Khay hệ thống</v-btn>
            ...
            <div class="mt-4 px-2">
              <div class="d-flex align-center justify-space-between mb-6 px-2">
                <div>
                  <div class="text-subtitle-2 text-grey mb-1">
                    Phiên bản UI/UX: <span class="font-weight-bold ml-1" :class="isDark ? 'text-white' : 'text-black'">{{ props.app?.FE_version }}</span>
                  </div>
                  <div class="text-subtitle-2 text-grey">
                    Phiên bản Client: <span class="font-weight-bold ml-1" :class="isDark ? 'text-white' : 'text-black'">{{ props.app?.BE_version || 'Không xác định' }}</span>
                  </div>
                </div>
                <v-btn color="primary" variant="tonal" class="rounded-lg update-btn px-5" height="48" @click="$emit('check-update')">
              ...
              <div class="d-flex align-center justify-space-between mb-6">
                <div>
                  ...
                  <div class="text-caption text-medium-emphasis">Tải lại dữ liệu ứng dụng sau khi mở rộng quyền truy cập</div>
                </div>
                <v-btn color="primary" variant="tonal" class="rounded-lg update-btn px-5" height="48" @click="$emit('get-apps')">
                  <v-icon start icon="mdi-refresh" size="24" class="update-icon"></v-icon>Cập nhật ngay
                </v-btn>
              </div>
              <v-divider class="mb-5 opacity-20"></v-divider>
              <div class="px-2 mb-4">
          ...
          <v-window-item value="application">
            <div class="mt-4 px-2">
              <div class="px-2 pb-4">
                <div class="text-h6 font-weight-bold mb-4"><v-icon icon="mdi-wrench-outline" start color="primary" size="small"></v-icon> Quản lý ứng dụng</div>
                <v-row>
                  <v-col cols="12" class="py-2">
                    <div class="d-flex align-center justify-space-between bg-surface-variant-light pa-3 rounded-lg border">
                      <div>
                        <div class="text-body-2 font-weight-bold">Dọn dẹp bộ nhớ đệm</div>
                        <div class="text-caption text-medium-emphasis">Giải phóng dữ liệu tạm (Sẽ khởi động chậm hơn vào lần sau)</div>
                      </div>
          ...
          <v-window-item value="about">
            <div class="text-center mt-4">
              ...
              <v-card variant="tonal" :color="isDark ? 'grey' : 'primary'" class="mt-8 pa-4 rounded-lg text-left">
                <div class="text-body-2 mb-1">Ứng dụng được phát triển bởi <strong>Hiếu GL Lite</strong> cùng với chatbot <strong>Gemini</strong></div>
                <div class="text-body-2 text-medium-emphasis">Dựa trên nền tảng .NET 8, Vue.js và Vuetify 3. Tối ưu hóa trải nghiệm ứng dụng một cách tốt nhất.</div>
              </v-card>
            </div>
          </v-window-item>
...
const props = defineProps(['app', 'settings']);
...
<style scoped>
.update-icon { transition: transform 0.6s cubic-bezier(0.4, 0, 0.2, 1); }
.update-btn:hover .update-icon { transform: rotate(180deg); }
