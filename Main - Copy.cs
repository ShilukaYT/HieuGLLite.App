...
namespace HieuGLLite.Apps
{
	public partial class Main : Form
	{
		...
		public Main()
		{
			...
			//Main.WriteLog("INFO", "Changing theme to " + currentTheme);
		...
		private async void Main_Load(object sender, EventArgs e)
		{
			...
			//WriteLog ("INFO", System.Globalization.RegionInfo.CurrentRegion.TwoLetterISORegionName);
		...
		{
			// KHIÊN BẢO VỆ: Nếu file JSON đã tồn tại, tức là khách đã mở app từ lần thứ 2 trở đi -> Quay xe, không làm gì cả
			...
			try
			{
				string countryCode = "US"; // Mặc định là Mỹ (Tiếng Anh)

				// ==========================================
				// CÁCH 1 (Khuyên dùng): Đọc mã Vùng của Windows (Nhanh 0.001s, không cần mạng)
				// ==========================================
				countryCode = System.Globalization.RegionInfo.CurrentRegion.TwoLetterISORegionName;

				// ==========================================
				// CÁCH 2 (Theo ý bạn): Dùng API IP Quốc tế
				// (Bỏ comment đoạn dưới nếu bạn bắt buộc muốn dùng API mạng)
				// ==========================================
				/*
				using (HttpClient client = new HttpClient())
				{
					// Thay URL này bằng link API bạn có
				...
				*/
				...
				// BẮT BUỘC: Lưu lại ngay để ép sinh ra file JSON, lần sau mở app sẽ không bị check lại nữa
				SaveSettings();
			}
			catch (Exception ex)
			{
				// Nếu API sập hoặc lỗi, fallback về tiếng Anh cho an toàn
				currentLanguage = "en-US";
				SaveSettings();
		...
		private void ApplyLanguage(string langCode)
		{
			try
			{
				// 1. Chuyển đổi mã ngôn ngữ
				var culture = new System.Globalization.CultureInfo(langCode);

				// 2. Ép toàn bộ luồng chạy của C# đổi sang ngôn ngữ mới
		...
		private void SystemEvents_UserPreferenceChanged(object sender, UserPreferenceChangedEventArgs e)
		{
			if (e.Category == UserPreferenceCategory.General)
			{
				if (currentTheme == "system")
				{
					...
					// 🛡️ CHỐT CHẶN: Nếu hệ thống báo đổi sang màu Dark, 
					// mà app cũng ĐANG LÀ Dark rồi thì quay xe luôn, không làm gì cả!
					if (_lastAppliedSystemThemeIsDark == isDark) return;

					// Cập nhật lại trí nhớ
		...
		private void WebView21_CoreWebView2InitializationCompleted(object sender, CoreWebView2InitializationCompletedEventArgs e)
		{
			...
			{
				CustomMessageBox.Show("WebView2 initialization failed: " + e.InitializationException, "Error", false);
			}
		}
    ...
    <video autoplay muted loop playsinline>
		...
		private void WebView_WebMessageReceived(object sender, CoreWebView2WebMessageReceivedEventArgs e)
		{
			...
			switch (type)
			{
				...
				case "OPEN_URL":
					...
					if (!string.IsNullOrEmpty(url))
					{
						...
						catch (Exception ex)
						{
							...
							CustomMessageBox.Show("Không thể mở liên kết: " + ex.Message, "Error", false);
						}
					}
					break;
					...
					Task.Run(() =>
					{
						try
						{
							...
							if (app != null && app.isInstalled && !string.IsNullOrEmpty(app.programPath))
							{
								...
								{
									CustomMessageBox.Show($"Không tìm thấy file thực thi tại: {exePath}", "Error", false);
						...
						{
							CustomMessageBox.Show($"Không thể khởi động ứng dụng: {ex.Message}", "Error", false);
						}
					});
					break;
				case "KILL_APP":
					...
					Task.Run(() =>
					{
						...
						{
							var result = CustomMessageBox.Show("Bạn có chắc muốn tắt ứng dụng này?\nTiến trình chơi của bạn có thể sẽ không được lưu lại!", "Xác nhận", true);
				...
				case "CANCEL_DOWNLOAD":
					var confirmResult = CustomMessageBox.Show("Bạn có chắc muốn hủy tải ứng dụng này?\nMọi tiến trình và dữ liệu tạm thời sẽ bị xóa sạch!", "Xác nhận hủy tải", true);
				...
				case "CLEAR_CACHE":
					...
					else
					{
						...
						CustomMessageBox.Show("Có tệp đang tải xuống, bạn không thể dọn dẹp ngay lúc này!", "Không thể dọn dẹp", false);
					}
					break;
				case "UNINSTALL_APP":
					...
					if (uninstallConfirm == DialogResult.Yes)
					{
						...
						{
							CustomMessageBox.Show("Cần quyền Quản trị viên (Run as Administrator) để gỡ bỏ hoàn toàn giao thức hệ thống.", "Thiếu quyền", false);
						}
						catch (Exception ex)
						{
							CustomMessageBox.Show($"Lỗi gỡ Registry: {ex.Message}", "Lỗi", false);
						}
				...
				case "CHANGE_LANGUAGE":
					// 1. Nhận mã ngôn ngữ từ Vue (ví dụ: "en" hoặc "vi")
					string newLang = message["lang"]?.ToString() ?? "vi";

					// 2. Cập nhật biến và lưu vào JSON
					currentLanguage = newLang;
					SaveSettings();

					// 3. Gọi hàm đổi ngôn ngữ
					this.Invoke((MethodInvoker)delegate
					{
						ApplyLanguage(currentLanguage);
					});
				...
				case "INSTALL_UPDATE":
					...
					{
						CustomMessageBox.Show("Không tìm thấy file cài đặt, vui lòng tải lại!", "Lỗi", false);
					}
					break;
				...
				//	break;
				case "default":
					CustomMessageBox.Show("Yêu cầu không hợp lệ: " + type, "Error", false);
					break;
			}
		}
		...
		private async Task CleanLauncherInternalFilesAsync()
		{
			try
			{
				...
				// 2. Dọn dẹp thư mục Logs (Xóa các tệp chưa hoàn thiện)
		...
		private void UninstallApp(string appId)
		{
			...
			{
				CustomMessageBox.Show($"Không tìm thấy công cụ: BSTCleaner.exe", "Error", false);
				return;
			}
			...
			Task.Run(() =>
			{
				...
				catch (Exception ex)
				{
					...
					CustomMessageBox.Show($"Lỗi thực thi: {ex.Message}", "Error", false);
					SendStatusToVue(appId, "DOWNLOAD_FAILED", new { error = ex.Message });
				}
			});
		...
		private bool CleanConflictingApp(GameApp app)
		{
			...
			if (!System.IO.File.Exists(fullPath))
			{
				this.Invoke((MethodInvoker)delegate
				{
					CustomMessageBox.Show($"Không tìm thấy công cụ dọn dẹp: BSTCleaner.exe", "Lỗi", false);
				});
				return false;
			}
			...
			catch (Exception ex)
			{
				this.Invoke((MethodInvoker)delegate
				{
					CustomMessageBox.Show($"Lỗi khi chạy công cụ dọn dẹp: {ex.Message}", "Error", false);
				});
				return false;
			}
		}
		private void ExecuteAppUtility(string appId, string status, string exeName, string args = "")
		{
			...
			if (!System.IO.File.Exists(fullPath))
			{
				CustomMessageBox.Show($"Không tìm thấy công cụ: {exeName}", "Error", false);
				return;
			}
		...
		private void HandleDiscordLogin()
		{
			...
			catch (Exception ex)
			{
				...
				CustomMessageBox.Show("Không thể mở trình duyệt: " + ex.Message, "Error", false);
			}
		}
		...
		private async void ExchangeCodeForTokenAsync(string code)
		{
			...
			using (HttpClient client = new HttpClient())
			{
				...
				{
					CustomMessageBox.Show("Đổi token thất bại: " + responseString, "Lỗi API", false);
				}
			}
		}
		...
		private void SaveSecureToken(string token)
		{
			...
			catch (Exception ex)
			{
				...
				CustomMessageBox.Show("Lỗi lưu token: " + ex.Message, "Error", false);
			}
		}
		...
		private async Task<List<string>> FetchUserRolesFromDiscord(string accessToken)
		{
			try
			{
				...
				else if (response.StatusCode == HttpStatusCode.NotFound)
				{
					...
					Task.Run(() =>
					{
						CustomMessageBox.Show("Bạn chưa tham gia máy chủ của Hiếu GL Lite\nTính năng của ứng dụng có thể sẽ bị hạn chế!", "Thông báo", false);
					});
				}
			}
			catch (Exception ex)
		...
		private void HandleInstallRequest(dynamic message)
		{
			...
			if (isDownloading)
			{
				...
				this.Invoke((MethodInvoker)delegate
				{
					CustomMessageBox.Show("Hiện đang có một ứng dụng khác đang được xử lý!", "Thông báo", false);
				});
				return;
			}
			...
			if (app.isVerifyRequired && !isSessionVerified)
			{
				...
				this.Invoke((MethodInvoker)delegate
				{
					CustomMessageBox.Show("Phát hiện truy cập trái phép!\nBạn chưa hoàn thành bước xác minh OTP.", "Bảo mật hệ thống", false);
				});
			...
			if (freeGB < requiredGB)
			{
				...
				this.Invoke((MethodInvoker)delegate
				{
					CustomMessageBox.Show($"Ổ đĩa {Path.GetPathRoot(checkPath)} không đủ dung lượng!\n" + $"- Cần: {requiredGB:F2} GB\n" + $"- Hiện có: {freeGB:F2} GB", "Cảnh báo", false);
				});
		...
		private async Task StartComboDownloadAsync(string appid, dynamic data)
		{
			...
			try
			{
				...
				if (!System.IO.File.Exists(exePath) || !System.IO.File.Exists(androidPath))
				{
					...
					CustomMessageBox.Show("Cài đặt thất bại, không tìm thấy tệp cần cài đặt!", "Cài đặt thất bại", false);
			...
			try
			{
				...
				using (Process p = new Process())
				{
					...
					else
					{
						...
						CustomMessageBox.Show("Cài đặt thất bại, trình cài đặt hiện đã hoàn tác thay đổi", "Cài đặt thất bại", false);
						SetTaskbarState(TaskbarProgressBarStatus.Error);
		...
		//	if (System.IO.File.Exists(statePath))
		//	{
		//		WriteLog("INFO", $"Resuming download");
